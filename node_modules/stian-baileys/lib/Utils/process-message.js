...
const processMessage = async (message, { shouldProcessHistoryMsg, placeholderResendCache, ev, creds, signalRepository, keyStore, logger, options, getMessage }) => {
    ...
    if (protocolMsg) {
         ...
         */
        const parseStubParticipants = (params) => {
            const out = [];
            for (const raw of params || []) {
                ...
                if (raw.includes('@')) {
                    out.push(isLidUser(raw)
                        ? { id: raw, lid: raw }
                        : { id: raw, phoneNumber: raw });
                }
                else {
        ...
        switch (message.messageStubType) {
            ...
            case WAMessageStubType.GROUP_MEMBERSHIP_JOIN_APPROVAL_REQUEST_NON_ADMIN_ADD: // TODO: Add other events
                ...
                const participant = parseStubParticipants(message.messageStubParameters?.slice(0, 1))[0];
                ...
                if (participant) {
                    ...
                    logger?.debug({ params: message.messageStubParameters }, 'skipping join approval request with unparsable participant');
                }
                break;
        }
